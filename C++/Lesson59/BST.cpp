...
class BST
{
    public:
        ...
        friend std::istream& operator>>(std::istream& input, BST& bst);
    ...
    private:
        ...
        void print(std::ostream& output, Node* temp)  const // It is inorder print 
        {
            if(temp != nullptr)
            {
                print(output, temp->left);
                output << temp->data << std::endl;
                print(output, temp->right);
            }
        }
...
std::ostream& operator<<(std::ostream& output, const BST& bst)
{
    bst.print(output);
    return output;
}

std::istream& operator>>(std::istream& input, BST& bst)
{
    std::cout << "Please enter numbers for inserting in to the binary search tree." << std::endl;
